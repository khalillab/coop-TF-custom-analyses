...
main = function(input_coverage='affinity-dependent-peaks-filtered-allsamples-allannotations-ZF-chipseq-spikenorm-protection.tsv.gz',
    ...
    df = read_tsv(input_coverage,
        ...
        # ungroup() %>%
        mutate(has_ad = !str_detect(group, 'noVP16'),
               group=str_remove(group, '-noVP16'),
               replicate=str_sub(sample, start=-1),
               group=ordered(group,
    ...
    df %<>% bind_rows(df %>% filter(group=='reporter-only') %>%
                          mutate(has_ad=FALSE))
